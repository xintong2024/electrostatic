...
function dispChooserChanged() {
  ...
  vecDensityLabel.innerHTML = (disp == "viewpaper" ? "分辨率:" : "向量密度:");
  var strengthLabel = document.getElementById("strengthLabel");
  strengthLabel.innerHTML = (disp.startsWith("parts")) ? "粒子速度:" : "亮度:";
  if (disp == "equip")
...
class InverseSquaredRadial extends VecFunction {
  getName() { return BUILD_CASE_EMV("点电荷", null, "1/r^2 single"); }
  getFieldMult() { return .0003/coulombKQ; }
  getPotMult() { return .1/coulombKQ; }
  getInfo() { return "电荷 = " + (-reverse) + " nC<br>"; }
  getField(result, y) {
    var r = distance(y);
    if (r < chargeSize)
      boundCheck = true;
...
class InverseSquaredRadialDouble extends InverseSquaredRadial {
  getName() { return BUILD_CASE_EMV("两个点电荷", null, "1/r^2 double"); }
  getBestSlice() { return SLICE_Y; }
  getInfo() {
     var sep = auxBarValue(1)/100.;
     return "电荷 = " + (this.sign2 == 1 ? -reverse : "&plusmn;1") + " nC<br>间隔 = " + round(sep*2) + " m<br>";
  }
  getField(result, y) {
    var sep = auxBarValue(1)/100.;
    var xx1 = y[0]-sep;
  ...
  createNext() { return BUILD_CASE_EMV(new InverseSquaredRadialDipole(), null, new InverseRadial()); }
	...
	getName() { return "四极子 "; }
        getInfo() {
          var sep = auxBarValue(1)/100.;
          return "电荷 = " + (this.sign2 == 1 ? "1" : "&plusmn;1") + " nC<br>间隔 = " + round(sep*2) + " m<br>";
        }
	getField(result, y) {
	    var sep = auxBarValue(1)/100.;
	    var xx1 = y[0] - sep;
...
class InverseRadial extends VecFunction {
  getName() { return BUILD_CASE_EMV("带电直线", null, "1/r single line"); }
  getFieldMult() { return .0003/(2*coulombKQ); }
  getPotMult() { return .4/(2*coulombKQ); }
  getInfo() { return "电荷 = " + (-reverse) + " nC/m<br>"; }
  getField(result, y) {
...
class InverseRadialDouble extends VecFunction {
  ...
  getName() { return BUILD_CASE_EMV("双带电直线", null, "1/r double lines"); }
  getFieldMult() { return .0003/(2*coulombKQ); }
  getPotMult() { return .2/(2*coulombKQ); }
  getInfo() {
     var sep = auxBarValue(1)/100.;
     return "电荷 = " + (this.sign == 1 ? -reverse : "&plusmn;1") + " nC/m<br> 间隔 = " + round(sep*2) + " m<br>";
  }
  getField(result, y) {
    var sep = auxBarValue(1)/100.;
    var xx1 = y[0] - sep;
  ...
  getInfo() {
     var sep = auxBarValue(1)/100.;
     return "电荷 = &plusmn;1 nC/m<br> 间隔 = " + round(sep*2) + " m<br>";
  }
	getField(result, y) {
	    var sep = auxBarValue(1)/100.;
	    var xx1 = y[0] + sep;
	...
	getName() { return "有穷直线"; }
        ...
        getPotMult() { return .1/coulombKQ; }
        getInfo() {
          return "总电荷: " + (-reverse) + " nC<br> 长度: " + round(this.lineHalfLen*2) + " m<br>";
        }
	setup() {
	    setXZView();
	    setupBar(0, "直线长度", 30);
	...
	setupFrame() {
	    ...
	    var sep = auxBarValue(2)/100.;
            return "每条线上电荷: " + (this.dipole == 1 ? -reverse : "&plusmn;1") + " nC<br>长度: " + round(this.lineHalfLen*2) + " m<br>" +
                 "间隔: " + round(sep*2) + " m<br>";
        }
	getField(result, y) {
	    var sep = auxBarValue(2)/100.;
	...
	constructor() {
        ...
        getInfo() {
            return "宽度: " + round(this.a*2) + " m<br>" + super.getInfo();
        }
	setupFrame() {
	...
	useRungeKutta() { return false; }
        getInfo() {
	    var sep = auxBarValue(2)/100.;
            return "间隔: " + round(sep*2) + " m<br>" + super.getInfo();
        }
	getField(result, y) {
	    var sep = auxBarValue(2)/100.;
	...
	getBestSlice() { return SLICE_Y; }
        getInfo() { return "球体半径: " + round(this.getSphereRadius()) + " m<br>" +
                           "球体位置: (" + round(this.getSpherePos()) + ", 0, 0)<br>" +
                           "点位置: (" + round(this.getPointPos()) + ", 0, 0)<br>" +
                           "间隔: " + round(this.getSeparation()) + " m<br>" +
                           this.chargeInfo() + super.getInfo(); }
        chargeInfo() { return "球体势能: " + round(auxBarValue(3)-50)*reverse*2 + " V<br>"; }
	setup() {
	    setupBar(0, "球体尺寸", 30);
	    setupBar(1, "间隔", 50);
	    setupBar(2, "球体电势", 50);
	...
	getName() { return "带电荷球体 + 点电荷"; }
        setup() {
          super.setup();
          this.image = 0;
          setupBar(2, "球体电荷", 50);
        }
        chargeInfo() { return "球体电荷: " + round(reverse*this.getSphereCharge()/coulombKQ) + " nC<br>"; }
	...
	setup() {
        ...
        getInfo() { return "圆柱半径: " + round(this.getCylRadius()) + " m<br>" +
                           "圆柱位置: (" + round(this.getCylPos()) + ", 0, z)<br>" +
                           "线位置: (" + round(this.getPointPos()) + ", 0, z)<br>" +
                           "间隔: " + round(this.getSeparation()) + " m<br>" +
                           "圆柱电势: " + reverse*round(auxBarValue(3)-50) + " V<br>"; }
        getFieldMult() { return .0003/(2*coulombKQ); }
	...
	getField(result, y) {
	    ...
	    var e1 = auxBarValue(2)/10. + 1;
	    var cx = auxBarValue(1)/50.-1.001;
            var s = "&epsilon;<sub>r</sub> = " + round(e1) + " 下方, 1 上方<br>"
            if (this.conducting) s = "";
            s += round(Math.abs(cx-this.planeZ));
            s += (this.conducting) ? " m 平面上方<br>" :
                 (cx > this.planeZ) ? " m 边界上方<br>" : " m 边界下方<br>";
	...
	getName() { return BUILD_CASE_EMV(null, "电流线",
				  "1/r 圈"); }
	setup() {
	    setXYViewExact();
	}
        getFieldMult() { return showA ? .001/2 : .0001/2; }
        getInfo() { return "电流 = " + (10*reverse) + " A<br>"; }
	getField(result, y) {
	    var r = distance(y[0], y[1]);
	    if (showA) {
		result[0] = result[1] = 0;
	...
	getName() { return BUILD_CASE_EMV(null, "双电流线", "1/r rotational double"); }
        getFieldMult() { return showA ? .001/2 : .0001/2; }
        getInfo() {
          var sep = auxBarValue(1)/100.;
          return "电流 = " + (this.sign == 1 ? "" : "&plusmn;") + "10 A<br>间隔 = " + round(sep*2) + " m<br>";
        }
	getField(result, y) {
	    var sep = auxBarValue(1)/100.;
	...
	getName() { return "移动电荷"; }
        getInfo() { return "电荷 = 10 C<br>Speed: 5 m/s<br>"; }
	getField(result, y) {
	    var rz = distance(y);
	    if (showA) {
		result[0] = result[1] = 0;
	...
	getName() { return "高速电荷"; } // E field!
	getBestSlice() { return SLICE_Y; }
        getInfo() {
	   var beta = (auxBarValue(1)+1)/102.;
           return "电荷 = 1 nC<br>Speed: " + round(beta) + " c<br>";
        }
	getField(result, y) {
	    var rz = distance(y);
	    if (rz < chargeSize)
	...
	render(g) {
        ...
        getInfo() { 
	    var beta = (auxBarValue(1)+1)/102.;
            return "电荷 = " + (-reverse) + " nC<br>速率 = " + round(beta) + " c";
        }
	nonGradient() { return false; }
	...
	getName() { return "高速电荷"; } // B field
        getInfo() {
	   var beta = (auxBarValue(1)+1)/102.;
           return "电荷 = 1 &mu;C<br>速率: " + round(beta) + " c<br>";
        }
	...
	getName() { return "双移动电荷"; }
        getInfo() { return "电荷 = 10 C<br>速率: 5 m/s<br>"; }
	constructor() { super(); this.dir2 = 1; }
	getField(result, y) {
	    result[0] = result[1] = result[2] = 0;
	    var sep = auxBarValue(1)/100.;
